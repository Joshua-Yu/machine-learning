...
def sample_gaussian(m, v):
    ...
    ### START CODE HERE ###
    std = torch.sqrt(v)
    epsilon = torch.randn_like(std)  # Sample epsilon from N(0,1)
    z = m + std * epsilon  # Calculate z using reparameterization trick
    return z
    ### END CODE HERE ###
    ################################################################################
...
def log_normal(x, m, v):
    ...
    ################################################################################
    ### START CODE HERE ###
     # Compute the log variance (to avoid recalculating log variance multiple times)
    log_var = torch.log(v)

    # Element-wise log probability of the normal distribution
    log_prob_elem = -0.5 * (log_var + torch.pow(x - m, 2) / v + torch.log(torch.tensor(2 * torch.pi)))

    # Sum over the last dimension (which represents the different dimensions of the Gaussian)
    log_prob = log_prob_elem.sum(dim=-1)
...
def log_normal_mixture(z, m, v):
    ...
    ### START CODE HERE ###
    batch_size, mix_size, dim_size = m.size()

    # Expand z to match the mixture size
    z_expanded = z.unsqueeze(1).expand(-1, mix_size, -1)  # (batch, mix, dim)

    # Compute log variance
    log_v = torch.log(v)

    # Compute log probability for each Gaussian component
    log_probs = -0.5 * (log_v + torch.pow(z_expanded - m, 2) / v + torch.log(torch.tensor(2 * torch.pi)))
                
    # Sum over the last dimension (features) to get the log probability for each mixture
    log_probs = log_probs.sum(dim=-1)  # (batch, mix)

    # Compute the log of the uniformly weighted mixture by summing in log-space
    log_mix_coeffs = torch.log(torch.tensor(1.0 / mix_size))
    log_prob = torch.logsumexp(log_probs + log_mix_coeffs, dim=-1)  # (batch,)
...
def log_normal_mixture2(z, m, v):
    ...
    log_probs = -0.5 * (log_v + torch.pow(z_expanded - m, 2) / v).mean(dim=-1)  # (batch, mix)

    log_probs = log_probs.sum(-1)  # (batch)
    #log_probs = log_probs.sum(-1)# / dim_size  # (batch, mix)

    # Uniformly weight the mixture components (log(1/K) = -log(K))
    log_prob = np.log(log_probs) - np.log(mix_size)  # (batch,)

    # Apply log-sum-exp to compute the total log probability
    #log_prob = torch.logsumexp(log_probs, dim=1)  # (batch,)
...
def gaussian_parameters(h, dim=-1):
    ...
    """
    # split output h into 2 parts: m and h
    m, h = torch.split(h, h.size(dim) // 2, dim=dim)

    # calculate variance and ensure it is positive and over 0
    v = F.softplus(h) + 1e-8
    return m, v
