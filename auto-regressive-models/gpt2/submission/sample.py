...
def temperature_scale(logits, model, new_past, config, temperature, temperature_horizon):
    ...
    # EXTRA CREDIT ONLY   
        ...
        for t in range(config.vocab_size):
            ...
            joint_prob_t = None
            ### START CODE HERE ###
            logits_next, _ = model(new_current_text, past=new_past)
            logits_next = logits_next[:, -1, :] # get the logits for the last token

            logits_next = top_k_logits(logits_next, k=config.top_k)

            probs_next = F.softmax(logits_next, dim=-1)
            joint_prob_t = first_prob * probs_next
        ...
        first_logits = torch.logsumexp(joint_logits_scaled, dim=-1) # marginalize out the second token dimension    
        ### END CODE HERE ###
...
def sample(model, start_text, config, length, temperature=None, temperature_horizon=1):
    ...
    with torch.no_grad():
        for _ in trange(length):
            ...
            #     new_past: the calculated hidden state of previous + current text

            current_logits = logits[:, -1, :]   # Keep only the logits for the last token
            ...
            # Append sample to output and return only 1 sample
            sample = torch.multinomial(probs, 1)
            output.append(sample)

            # Update current_text so that sampling can continue
            current_text = sample
